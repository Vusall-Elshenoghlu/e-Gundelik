...
import { Container, Row, Col, Button, Card } from 'react-bootstrap'; // Bootstrap komponentləri
import styles from './StudentDiary.module.css';

// Təqvimin ikonası üçün sadə SVG (daha real layihələrdə react-icons istifadə oluna bilər)
const CalendarIcon = () => (
...
// Məlumatlar (dinamik olaraq API-dən gələ bilər)
const timetable = {
  '16.06.2025 Bazar ertəsi': [
    'Azərbaycan dili', 'Azərbaycan dili', 'Riyaziyyat', 'Musiqi', '', '', ''
  ],
  '17.06.2025 Çərşənbə axşamı': [
    'Azərbaycan dili', 'Azərbaycan dili', 'Riyaziyyat', 'İngilis dili', 'Fiziki tərbiyə', '', ''
  ],
  '18.06.2025 Çərşənbə': [
    'Azərbaycan dili', 'Azərbaycan dili', 'Riyaziyyat', 'Təsviri incəsənət', 'İnformatika', '', ''
  ],
  '19.06.2025 Cümə axşamı': [
    'Azərbaycan dili', 'Azərbaycan dili', 'Riyaziyyat', 'Texnologiya', '', '', ''
  ],
  '20.06.2025 Cümə': [ // Əlavə gün əlavə etdim ki, cədvəl daha dolğun görünsün
    'Azərbaycan dili', 'Azərbaycan dili', 'Fiziki tərbiyə', 'Musiqi', '', '', ''
  ]
};
    ...
    >
      <Container fluid className={styles.mainContent}>
        ...
        <Row className={`align-items-center mb-4 ${styles.header}`}>
          {/* Görünüş Seçicisi (Həftə/Gün Cədvəli) */}
        ...
        <Card className={styles.tableCard}>
          <Card.Body className="p-0">
            <div className={styles.tableWrapper}>
              {/* Cədvəl Başlığı (Saatlar) */}
              <div className={styles.tableGridHeader}>
                <div className={styles.emptyCell}></div> {/* Sol yuxarı boş xana */}
                {hours.map((hour, i) => (
                  <div key={i} className={styles.hourCell}>{hour}</div>
                ))}
              </div>

              {/* Cədvəl Sıraları (Günlər və Dərslər) */}
